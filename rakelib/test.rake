...
class AsanTestTask < ValgrindTestTask
  def ruby(*args, **options, &block)
    command = "ASAN=1 #{RUBY} #{args.join(" ")}"
    sh(command, **options, &block)
  end
end
...
namespace "test" do
  Rake::TestTask.new("bench") do |test_config|
    selma_test_bench_configuration(test_config)
  end
  ...
  GdbTestTask.new("gdb") do |test_config|
    selma_test_case_configuration(test_config)
  end

  LldbTestTask.new("lldb") do |test_config|
    selma_test_case_configuration(test_config)
  end

  AsanTestTask.new("asan") do |test_config|
    selma_test_case_configuration(test_config)
  end
